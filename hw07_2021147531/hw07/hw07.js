...
async function initShader() {
    const gVertexShaderSource = await readShaderFile('shGVert.glsl');
    const gFragmentShaderSource = await readShaderFile('shGFrag.glsl');
    gShader = new Shader(gl, gVertexShaderSource, gFragmentShaderSource);

    const pVertexShaderSource = await readShaderFile('shPVert.glsl');
    const pFragmentShaderSource = await readShaderFile('shPFrag.glsl');
    pShader = new Shader(gl, pVertexShaderSource, pFragmentShaderSource);
...
async function main() {
    try {
        ...
        pShader.use();
        pShader.setMat4("u_projection", projMatrix);

        pShader.setVec3("material.diffuse", vec3.fromValues(1.0, 0.5, 0.31));
        pShader.setVec3("material.specular", vec3.fromValues(0.5, 0.5, 0.5));
        pShader.setFloat("material.shininess", 32);

        pShader.setVec3("light.position", lightPos);
        pShader.setVec3("light.ambient", vec3.fromValues(0.2, 0.2, 0.2));
        pShader.setVec3("light.diffuse", vec3.fromValues(0.7, 0.7, 0.7));
        pShader.setVec3("light.specular", vec3.fromValues(1.0, 1.0, 1.0));
        pShader.setVec3("u_viewPos", cameraPos);

        gShader.use();
        gShader.setMat4("u_projection", projMatrix);

        gShader.setVec3("material.diffuse", vec3.fromValues(1.0, 0.5, 0.31));
        gShader.setVec3("material.specular", vec3.fromValues(0.5, 0.5, 0.5));
        gShader.setFloat("material.shininess", 32);

        gShader.setVec3("light.position", lightPos);
        gShader.setVec3("light.ambient", vec3.fromValues(0.2, 0.2, 0.2));
        gShader.setVec3("light.diffuse", vec3.fromValues(0.7, 0.7, 0.7));
        gShader.setVec3("light.specular", vec3.fromValues(1.0, 1.0, 1.0));
        gShader.setVec3("u_viewPos", cameraPos);
